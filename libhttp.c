...
struct http_request *http_request_parse(int fd) {
  struct http_request *request = malloc(sizeof(struct http_request)); //should be ok
  if (!request) http_fatal_error("Malloc failed"); //should be ok

  char *read_buffer = malloc(LIBHTTP_REQUEST_MAX_SIZE + 1); //should be ok
  if (!read_buffer) http_fatal_error("Malloc failed"); //should be ok
